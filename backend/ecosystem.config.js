...
module.exports = {
  ...
  deploy: {
    production: {
      ...
      path: DEPLOY_PATH,
      env: {
        PATH: '/home/fireok/.nvm/versions/node/v16.20.2/bin/node', // <-- путь из `which`
      },
      ...
      'post-deploy': `cp ${DEPLOY_PATH}/shared/.env . || true && cp ${DEPLOY_PATH}/shared/.env.deploy . || true && npm ci && npm run build && pm2 reload ${DEPLOY_PATH}/current/ecosystem.config.js --env production || pm2 start ecosystem.config.js --env production`,
    },
  },
};
